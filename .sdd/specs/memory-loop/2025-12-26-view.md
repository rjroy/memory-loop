...
- **REQ-F-19**: System must parse and render wiki-links in the format `[[note-name]]` and `[[note-name|display text]]` as clickable elements
- **REQ-F-20**: System must navigate to the target note when a wiki-link is clicked, resolving relative paths based on current file location
- **REQ-F-21**: System must auto-append `.md` extension to wiki-link targets if not present
- **REQ-F-22**: System must render external links (http/https) with `target="_blank"` and `rel="noopener noreferrer"`
- **REQ-F-23**: System must display a breadcrumb navigation trail showing the current file's path (e.g., Root / docs / guide.md)
- **REQ-F-24**: System must allow users to click breadcrumb segments to navigate to parent directories
- **REQ-F-25**: System must prepend asset base URL to relative image paths (default: `/vault/assets`)
- **REQ-F-26**: System must preserve absolute image URLs without modification
- **REQ-F-27**: System must display truncation warning when file exceeds 1MB size limit
- **REQ-F-28**: System must show loading skeleton while file content is being fetched
- **REQ-F-29**: System must display error message when file cannot be read (e.g., "File not found")
- **REQ-F-30**: System must show empty state message ("Select a file to view its content") when no file is selected
- **REQ-F-31**: System must automatically load file content when navigating from external links (e.g., RecentActivity View button)

### File Content Editing (Adjust Mode)

- **REQ-F-32**: System must display an "Adjust" button in the MarkdownViewer header when a file is loaded
- **REQ-F-33**: Clicking "Adjust" must replace the rendered markdown with a plain `<textarea>` containing the raw file content
- **REQ-F-34**: System must display "Save" and "Cancel" buttons while in adjust mode
- **REQ-F-35**: "Save" must write the textarea content back to the vault file and return to view mode
- **REQ-F-36**: "Cancel" must discard changes and return to view mode without saving
- **REQ-F-37**: Pressing Escape while in adjust mode must trigger Cancel behavior
- **REQ-F-38**: System must track adjust mode state (`isAdjusting: boolean`) in the browser context
- **REQ-F-39**: System must store pending content (`adjustContent: string`) while editing
- **REQ-F-40**: System must clear adjust state when navigating to a different file
- **REQ-F-41**: System must display inline error if save fails (permissions, path issues)
- **REQ-F-42**: System must preserve textarea content on error so user can retry or copy content

### Responsive Layout

- **REQ-F-43**: System must display split-pane layout on desktop (tree on left, viewer on right) using CSS Grid
- **REQ-F-44**: System must allow users to collapse the tree pane on desktop via toggle button
- **REQ-F-45**: System must adjust viewer width when tree pane is collapsed
- **REQ-F-46**: System must hide desktop tree pane on mobile viewports (typically < 768px)
- **REQ-F-47**: System must show hamburger menu button in viewer header on mobile
- **REQ-F-48**: System must display tree as slide-out overlay when menu button is tapped on mobile
- **REQ-F-49**: System must show backdrop overlay behind mobile tree to indicate modal state
- **REQ-F-50**: System must dismiss mobile tree when backdrop is tapped
- **REQ-F-51**: System must dismiss mobile tree when close button is tapped
- **REQ-F-52**: System must automatically dismiss mobile tree when a file is successfully loaded

### Path Security

- **REQ-F-53**: Backend must validate all file/directory paths are within vault boundary before accessing filesystem
- **REQ-F-54**: Backend must reject path traversal attempts (e.g., `../`, `../../etc/passwd`) with `PATH_TRAVERSAL` error code
- **REQ-F-55**: Backend must use `realpath` resolution to detect symlinks and boundary escapes
- **REQ-F-56**: Backend must only allow reading/writing markdown files (`.md` or `.MD` extension, case-insensitive)
- **REQ-F-57**: Backend must return `INVALID_FILE_TYPE` error for non-markdown file access attempts
- **REQ-F-58**: Backend must return `FILE_NOT_FOUND` error when requested file doesn't exist
- **REQ-F-59**: Backend must return `DIRECTORY_NOT_FOUND` error when requested directory doesn't exist

### WebSocket Protocol

- **REQ-F-60**: Client must send `list_directory` message with relative path to request directory listing
- **REQ-F-61**: Server must respond with `directory_listing` message containing array of FileEntry objects
- **REQ-F-62**: Client must send `read_file` message with relative path to request file content
- **REQ-F-63**: Server must respond with `file_content` message containing content and truncation flag
- **REQ-F-64**: Client must send `write_file` message with relative path and content to save file
- **REQ-F-65**: Server must respond with `file_written` message containing path and success status
- **REQ-F-66**: Server must send `error` message with appropriate error code when operations fail
- **REQ-F-67**: Client must handle WebSocket reconnection by re-sending vault selection and reloading current directory
- **REQ-F-68**: Client must wait for `session_ready` message before requesting directory listings
