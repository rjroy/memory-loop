...
- **REQ-F-23**: System must display permission dialog when AI requests to use a tool (via `canUseTool` callback)
- **REQ-F-24**: Permission dialog must show tool name and input parameters for user review
- **REQ-F-25**: User can approve or deny each tool invocation
- **REQ-F-26**: User can select "Always allow" for a specific tool to skip future prompts for that tool
- **REQ-F-27**: "Always allow" preferences must persist in localStorage per vault
- **REQ-F-28**: System must pause AI response streaming while awaiting user permission decision

### Draft Persistence

- **REQ-F-29**: System must save input text to localStorage (`memory-loop-discussion-draft`) on every change
- **REQ-F-30**: System must restore draft from localStorage on component mount
- **REQ-F-31**: System must clear localStorage draft after successful message submission
- **REQ-F-32**: Inspiration prefill must take precedence over localStorage draft

### Slash Command Detection

- **REQ-F-33**: System must detect messages starting with "/" as slash commands
- **REQ-F-34**: System must display "Slash command detected" hint when input starts with "/"
- **REQ-F-35**: System must send slash commands to backend without special processing (backend handles interpretation)

### Connection Handling

- **REQ-F-36**: System must send `select_vault` message on WebSocket connect
- **REQ-F-37**: System must re-send vault selection on WebSocket reconnect
- **REQ-F-38**: System must handle `SESSION_NOT_FOUND` errors by clearing stale session ID and starting fresh
- **REQ-F-39**: System must abort in-flight queries when starting new query or closing connection

### Inspiration Integration

- **REQ-F-40**: System must populate input with inspiration text when `discussionPrefill` is set
- **REQ-F-41**: System must clear `discussionPrefill` from context after populating input
- **REQ-F-42**: Inspiration prefill must NOT auto-submit (user must click send)
...
16. **Permission dialog shown**: AI requests to use Read tool → Permission dialog appears showing "Read" and file path input
17. **Approve tool use**: User clicks "Allow" → Tool executes, response continues streaming
18. **Deny tool use**: User clicks "Deny" → Tool is skipped, AI continues without that tool result
19. **Always allow option**: User clicks "Always allow for Read" → Future Read tool requests auto-approved without prompt
20. **Always allow persistence**: User selects "Always allow for Read", refreshes page → Read tool requests still auto-approved
21. **Response paused during permission**: AI streams response, requests tool → Streaming pauses until user responds to permission dialog

### Draft Persistence

22. **Save draft on input change**: User types "draft message" but doesn't send → Refreshes page → Input field contains "draft message"
23. **Clear draft after send**: User types message, clicks Send → LocalStorage draft cleared
24. **Prefill takes precedence**: LocalStorage has "old draft", inspiration sets prefill "new prompt" → Input shows "new prompt", prefill cleared from context

### Error Handling

25. **No vault selected error**: User sends message before selecting vault → Error message "No vault selected. Send select_vault first."
26. **SDK error mapping**: SDK throws ENOENT error → Frontend displays "Claude Code executable not found. Please ensure Claude Code is installed."
27. **Abort in-flight query**: User sends message A, immediately sends message B before A completes → Query A aborted, query B starts fresh

### Slash Commands

28. **Detect slash command**: User types "/help" → Hint "Slash command detected" appears above input
29. **Send slash command**: User sends "/search pattern" → Backend receives discussion_message with "/search pattern" text
