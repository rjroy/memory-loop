...
describe("HomeView", () => {
  ...
  describe("accessibility", () => {
    it("has proper section landmarks", () => {
      ...
      expect(screen.getByLabelText("Vault information")).toBeTruthy();
...
describe("getSessionButtons", () => {
  // Helper to create prep status
  const prepExists: DailyPrepStatus = { exists: true };
  const noPrep: DailyPrepStatus | null = null;

  describe("Daily buttons", () => {
    it("shows Daily Prep when no prep file exists", () => {
      const buttons = getSessionButtons(new Date("2026-01-15"), noPrep);

      expect(buttons.some((b: DebriefButton) => b.label === "Daily Prep")).toBe(true);
      expect(buttons.find((b: DebriefButton) => b.label === "Daily Prep")?.command).toBe(
        "/daily-prep"
      );
    });

    it("shows Daily Debrief when prep file exists", () => {
      const buttons = getSessionButtons(new Date("2026-01-15"), prepExists);

      expect(buttons.some((b: DebriefButton) => b.label === "Daily Debrief")).toBe(true);
      expect(buttons.find((b: DebriefButton) => b.label === "Daily Debrief")?.command).toBe(
        "/daily-debrief"
      );
    });

    it("does not show both Daily Prep and Daily Debrief", () => {
      const buttonsWithPrep = getSessionButtons(new Date("2026-01-15"), prepExists);
      const buttonsNoPrep = getSessionButtons(new Date("2026-01-15"), noPrep);

      expect(buttonsWithPrep.some((b: DebriefButton) => b.label === "Daily Prep")).toBe(false);
      expect(buttonsNoPrep.some((b: DebriefButton) => b.label === "Daily Debrief")).toBe(false);
    });
  });
  ...
  describe("Weekly Debrief", () => {
    it("shows Weekly Debrief on Friday", () => {
      ...
      const buttons = getSessionButtons(new Date("2026-01-02"), noPrep);

      expect(buttons.some((b: DebriefButton) => b.label === "Weekly Debrief")).toBe(true);
      expect(buttons.find((b: DebriefButton) => b.label === "Weekly Debrief")?.command).toBe(
        "/weekly-debrief"
      );
    });
    ...
    it("shows Weekly Debrief on Saturday", () => {
      ...
      const buttons = getSessionButtons(new Date("2026-01-03"), noPrep);
    ...
    it("shows Weekly Debrief on Sunday", () => {
      ...
      const buttons = getSessionButtons(new Date("2026-01-04"), noPrep);
    ...
    it("does not show Weekly Debrief on Monday", () => {
      ...
      const buttons = getSessionButtons(new Date("2026-01-05"), noPrep);

      expect(buttons.some((b: DebriefButton) => b.label === "Weekly Debrief")).toBe(false);
    });

    it("does not show Weekly Debrief on Thursday", () => {
      // 2026-01-01 is a Thursday
      const buttons = getSessionButtons(new Date("2026-01-01"), noPrep);
  ...
  describe("Monthly Summary", () => {
    it("shows Monthly Summary on first day of month (summarizes previous month)", () => {
      // 2026-02-01
      const buttons = getSessionButtons(new Date("2026-02-01"), noPrep);

      const monthlyBtn = buttons.find((b: DebriefButton) => b.label === "Monthly Summary");
    ...
    it("shows Monthly Summary on third day of month", () => {
      // 2026-02-03
      const buttons = getSessionButtons(new Date("2026-02-03"), noPrep);

      expect(buttons.some((b: DebriefButton) => b.label === "Monthly Summary")).toBe(true);
    });

    it("does not show Monthly Summary on fourth day of month", () => {
      // 2026-02-04
      const buttons = getSessionButtons(new Date("2026-02-04"), noPrep);

      expect(buttons.some((b: DebriefButton) => b.label === "Monthly Summary")).toBe(false);
    });

    it("shows Monthly Summary on last day of month (summarizes current month)", () => {
      // 2026-01-31 (January has 31 days)
      const buttons = getSessionButtons(new Date("2026-01-31"), noPrep);
    ...
    it("shows Monthly Summary on second-to-last day of month", () => {
      // 2026-01-30
      const buttons = getSessionButtons(new Date("2026-01-30"), noPrep);
    ...
    it("handles February end (28 days in non-leap year)", () => {
      ...
      const buttons = getSessionButtons(new Date("2026-02-28"), noPrep);
    ...
    it("handles February 29 in a leap year", () => {
      ...
      const buttons = getSessionButtons(new Date("2028-02-29"), noPrep);
  ...
  describe("multiple buttons", () => {
    it("can show Daily Prep, Weekly, and Monthly on Sunday, Jan 3rd without prep", () => {
      // 2027-01-03 is a Sunday, first 3 days of month
      const buttons = getSessionButtons(new Date("2027-01-03"), noPrep);
    ...
    it("can show Daily Debrief, Weekly, and Monthly on Sunday, Jan 3rd with prep", () => {
      // 2027-01-03 is a Sunday, first 3 days of month, with prep
      const buttons = getSessionButtons(new Date("2027-01-03"), prepExists);
    ...
    it("shows only Daily Prep on a mid-week, mid-month day without prep", () => {
      // 2026-01-15 is a Thursday, middle of month
      const buttons = getSessionButtons(new Date("2026-01-15"), noPrep);

      expect(buttons.length).toBe(1);
      expect(buttons[0].label).toBe("Daily Prep");
    });
  });
});

describe("HomeView button interaction", () => {
  it("renders Daily Prep button when no vault is selected", () => {
    render(<HomeView />, { wrapper: Wrapper });

    // Should show Daily Prep by default (no prep status loaded yet)
    expect(screen.getByText("Daily Prep")).toBeTruthy();
  });

  it("switches to discussion mode with command when button is clicked", () => {
    render(<HomeView />, { wrapper: Wrapper });

    const button = screen.getByText("Daily Prep");
